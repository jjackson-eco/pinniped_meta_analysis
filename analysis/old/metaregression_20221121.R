...
## Interaction data
pinnrev <- read_csv("../../Data/scoping_data_extraction_20221122.csv") %>% 
  mutate(spp = paste0(genus, " ", species)) %>% 
  ## Standardising study names - FIX
...
op_interaction <- pinnrev %>% 
  ...
  dplyr::select(acc_no, study, study_lab, obs_no, author_type = auth_type,
...
int_plot <- ggplot(int_pred, aes(x = pred, y = study)) +
  ...
  annotate(geom = "rect", xmin = int_pred_ovr$lwr[2], 
           xmax = int_pred_ovr$upr[2], ymin = -0.1, ymax = 23, 
...
het_int <- tibble(Q = paste0(round(int_mod_base$Q, 1), " (p < 0.001)"),
  ...
  save_as_image("../../Figures/extraction_20221121/interaction_heterogeneity.png")
...
het_dmg <- tibble(Q = paste0(round(dmg_mod_base$Q, 1), " (p < 0.001)"),
  ...
  save_as_image("../../Figures/extraction_20221121/damage_heterogeneity.png")
...
ggsave(fig1, filename = "../../Figures/extraction_20221121/meta_analysis_summary.pdf",
...
fig_3b <- ggplot(int_dat, aes(x = gear_type, y = interaction_prop)) +
  ...
  annotate(geom = "text", label = "*", x = 4, y = 1.06, size = 8) +
...
ggsave((fig_3a + fig_3b)/ (fig_3c + fig_3d), filename = "../../Figures/extraction_20221121/fig3.pdf",
       width = 30, height = 25, units = "cm")

ggsave((fig_3a + fig_3b)/ (fig_3c + fig_3d), filename = "../../Figures/extraction_20221121/fig3.jpeg",
...
ggsave(pop_int / pop_dmg, filename = "../../Figures/extraction_20221121/pop_int_dmg.pdf",
       width = 10, height = 12)
...
ggsave(sp_int /sp_dmg, 
       filename = "../../Figures/extraction_20221121/species_int_dmg.pdf",
       width = 10, height = 17)
...
ggsave(fig_S1a + fig_S1b, filename = "../../Figures/extraction_20221121/figS1.jpeg",
