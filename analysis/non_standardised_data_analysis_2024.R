...
survey_summary <- nonstn_data %>% 
  ...
  summarise(study_name = study_name[1],
            ...
            modality = modality[1]) %>% 
  ungroup() %>% 
  mutate(modality = gsub("rerporting","reporting", modality),
         modality = gsub("Proportion of fishers", "Questionnaire", modality),
         modality = gsub("Self reporting", "Interview", modality))
