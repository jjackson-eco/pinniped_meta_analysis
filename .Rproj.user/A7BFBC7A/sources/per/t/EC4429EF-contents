...
int_pred <- tibble(study = int_mod_base_summary$studlab,
                   ...
                   lwr = boot::inv.logit(int_mod_base_summary$lower),
                   upr = boot::inv.logit(int_mod_base_summary$upper))
...
dmg_pred <- tibble(study = dmg_mod_base_summary$studlab,
                   ...
                   lwr = boot::inv.logit(dmg_mod_base_summary$lower),
                   upr = boot::inv.logit(dmg_mod_base_summary$upper))
...
het_int <- tibble(Q = paste0(round(int_mod_base$Q, 1), " (p < 0.001)"),
  ...
  save_as_image("output/interaction_heterogeneity.png")
...
het_dmg <- tibble(Q = paste0(round(dmg_mod_base$Q, 1), " (p < 0.001)"),
  ...
  save_as_image("output/damage_heterogeneity.png")
...
ggsave(fig2, filename = "output/meta_analysis_summary.pdf",
...
ggplot(int_dat, aes(x = pop_trend, y = interaction_prop)) +
  geom_jitter(width = 0.1, colour = int_colour)
...
int_pop <- metareg(int_mod_base, ~ pop_trend)
...
fig_3a <- ggplot(int_dat, aes(x = fishery_type, y = interaction_prop)) +
  ...
  geom_point(data = int_fish_pred_ovr, aes(y = pred), 
             size = 5) +
  annotate(geom = "text", label = "**", x = 2, y = 1.07, size = 8) +
...
ggplot(dmg_dat, aes(x = location, y = damage_prop)) +
  geom_jitter(width = 0.1, colour = dmg_colour)

ggplot(dmg_dat, aes(x = gear_type, y = damage_prop)) +
  geom_jitter(width = 0.1, colour = dmg_colour)

ggplot(dmg_dat, aes(x = pop_trend, y = damage_prop)) +
  geom_jitter(width = 0.1, colour = dmg_colour)

# Models
dmg_fishery <- metareg(dmg_mod_base, ~ fishery_type)
dmg_location <- metareg(dmg_mod_base, ~ location)
dmg_gear <- metareg(dmg_mod_base, ~ gear_type)
dmg_pop <- metareg(dmg_mod_base, ~ pop_trend)
...
ggsave((fig_3a + fig_3b)/ (fig_3c + fig_3d), filename = "output/fig3_noicons.pdf",
...
ggsave(pop_int + pop_dmg, filename = "output/pop_int_dmg.pdf",
...
ggsave(sp_int / sp_dmg, 
       filename = "output/species_int_dmg.pdf",
...
ggsave(fig_S3a + fig_S3b, filename = "output/figS3.pdf",
       width =25, height = 12.5, units = "cm")

##______________________________________________________________________________
#### 5. Meta-regression tables ####

int_coef_dat <- 
  bind_rows(coef(summary(int_fishery)),
          coef(summary(int_location)),
          coef(summary(int_gear)),
          coef(summary(int_pop))) %>% 
  mutate(term = rownames(.))

dmg_coef_dat <- 
  bind_rows(coef(summary(dmg_fishery)),
            coef(summary(dmg_location)),
            coef(summary(dmg_gear)),
            coef(summary(dmg_pop))) %>% 
  mutate(term = rownames(.))

# CSVs converted to excel sheets for formatting
# write_csv(int_coef_dat, file = "../../Figures/Manuscript_figures/tableS5.csv")
# write_csv(dmg_coef_dat, file = "../../Figures/Manuscript_figures/tableS6.csv")
